                ...
                try:
                    ...
                    if calc_psych:
                        for spk, stats in psych_stats.items():
                            ...
                            neg = stats.get("criticism", 0) + stats.get("contempt", 0) + stats.get("defensiveness", 0) + stats.get("stonewalling", 0) + stats.get("dark_triad", 0)
...
with tab_jobs:
    ...
    for job in jobs:
        with st.container():
            col_info, col_status, col_view, col_add, col_del = st.columns([3.5, 2.5, 1.2, 1.5, 1.2])
            with col_info:
            ...
            with col_view:
                if job['status'] == "Completed":
                    if st.button("👁️ View", key=f"view_{job['id']}", type="primary", use_container_width=True):
            ...
            with col_add:
                if job['status'] == "Completed" and not (job.get('calc_volume') and job.get('calc_sentiment') and job.get('calc_psych')):
                    if st.button("✨ Add Analysis", key=f"retro_{job['id']}", use_container_width=True):
...
with tab_help:
    ...
    *   **Four Horsemen:** Scans for John Gottman's relationship killers: Criticism, Contempt, Defensiveness, and Stonewalling. 
    *   **Gottman Ratio:** The total amount of "Positive/Restorative" words divided by "Negative" words (Horsemen + Dark Triad). *Healthy relationships naturally sit at around a 5:1 positive-to-negative ratio during conflict.*
    *   **I/You/We Talk:** 
        ...
        with tabs[0]:
            for seg in segments:
                ...
                for f in seg.get("psych_flags", []):
                    extra_class = ""
                    if f in ["You Talk", "Criticism", "Contempt", "Defensiveness", "Stonewalling"]: extra_class = "flag-red"
                    elif f == "We Talk": extra_class = "flag-green"
