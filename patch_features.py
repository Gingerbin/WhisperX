...
if "--worker" in sys.argv:
    ...
    while True:
        ...
        try:
            ...
            del diarize_model; cleanup_memory()
            ...
            if calc_volume or calc_sentiment or calc_psych:
                ...
                try:
                    ...
                    for i, seg in enumerate(segments):
                        ...
                        speaker = seg.get("speaker", "UNKNOWN")
                        t_lower = text.lower()
                        if calc_volume: calculate_audio_metrics(seg, full_audio, i, segments)
                        if analyzer is not None:
                            vs = analyzer.polarity_scores(text)
                            compound = vs['compound']
                            if compound >= 0.05: sentiment = "Positive"
...
<style>
    ...
    .segment-card { padding: 1rem; border-radius: 0.5rem; background-color: rgba(128, 128, 128, 0.1); margin-bottom: 1rem; border-left: 5px solid; transition: all 0.2s ease; }
    .timestamp { font-size: 0.8em; color: #888; font-family: monospace; vertical-align: middle; margin-right: 5px; }
    
    /* Play and Toggle button styling */
    .inline-play {
    ...
    .toggle-btn {
        cursor: pointer; background-color: #f39c12; color: white; border-radius: 4px;
        padding: 2px 6px; font-size: 10px; margin-left: 8px; vertical-align: middle; user-select: none; text-decoration: none !important;
    }
    .toggle-btn:hover { background-color: #e67e22; }
    ...
    /* Sticky Audio Player */
    [data-testid="stAudio"] { position: fixed; bottom: 0; left: 0; width: 100%; z-index: 99999; background-color: #1e1e1e; padding: 10px 20px; box-shadow: 0px -5px 15px rgba(0,0,0,0.5); }
...
window.parent.document.addEventListener('click', function(e) {
    ...
    if(e.target && e.target.classList.contains('toggle-btn')) {
        ...
        var el = window.parent.document.getElementById(targetId);
        if(el) {
...
// Active Segment Highlighter Poller
    ...
    if(audios.length > 0 && !audios[0].paused) {
        ...
        cards.forEach(function(c) {
            ...
            if(ct >= start && ct <= end) {
                ...
                c.style.backgroundColor = 'rgba(128, 128, 128, 0.1)';
            }
        });
    }
}, 250);
    ...
    for job in jobs:
        with st.container():
            ...
            with col_actions:
                ...
                # Forced horizontal inline buttons via columns
    ...
    if os.path.exists(result_file):
        ...
        with tabs[0]:
            for i, seg in enumerate(segments):
                ...
                metrics_html = f"<div style='margin-bottom: 8px;'>{' | '.join(m_parts)}</div>" if m_parts else ""
