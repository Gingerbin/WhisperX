...
PSYCH_DICT = {
    ...
    "four_horsemen_criticism": [r"\balways\b", r"\bnever\b", r"\bselfish\b", r"\bcomplain\b", r"\bfault\b"],
    "four_horsemen_contempt": [r"\bidiot\b", r"\bstupid\b", r"\bdisgusting\b", r"\bhate\b", r"\bsarcasm\b", r"\bwhatever\b"],
    "four_horsemen_defensiveness": [r"\bbut\b", r"\bexcuse\b", r"\bwasn't me\b", r"\bnot my fault\b", r"\byou too\b"],
    "four_horsemen_stonewalling": [r"\bfine\b", r"\bi'm done\b", r"\bleave me alone\b"],
    ...
    "attach_anxious": [r"\bworry\b", r"\babandon\b", r"\bleave me\b", r"\bneed you\b", r"\bclingy\b", r"\bafraid\b", r"\bcare about me\b"],
    "attach_avoidant": [r"\bspace\b", r"\bback off\b", r"\balone\b", r"\bindependent\b", r"\bmy own\b"]
}
    ...
    while True:
        ...
        def calculate_audio_metrics(seg, full_audio, i, all_segments):
            ...
            if slice_dbfs:
                ...
                third = max(1, len(slice_dbfs) // 3)
                start_dbfs = sum(slice_dbfs[:third]) / third
                end_dbfs = sum(slice_dbfs[-third:]) / third
                fade_out = (start_dbfs - end_dbfs) > 5.0
            ...
            if fade_out: seg["audio_flags"].append("Fade Out")
                ...
                try:
                    ...
                    if calc_psych:
                        for spk, stats in psych_stats.items():
                            pos = stats["restorative_positive"]
                            neg = stats["four_horsemen_criticism"] + stats["four_horsemen_contempt"] + stats["four_horsemen_defensiveness"] + stats["four_horsemen_stonewalling"] + stats["dark_triad"]
...
st.set_page_config(page_title="WhisperX Diarizer", page_icon="🎙️", layout="wide", initial_sidebar_state="expanded")
...
tab_jobs, tab_upload = st.tabs(["📋 Active & Past Jobs", "📁 Start New Job"])
...
with tab_upload:
    ...
    with col_t2:
        ...
        calc_psych = st.checkbox("🧠 Psychological Word Scan (Gottman, Attachment, Dark Triad)", value=True)

    if st.button("🚀 Start Background Job", type="primary", use_container_width=True, disabled=(audio_file is None)):
                ...
                with b1:
                    if job['status'] == "Completed":
                        if st.button("👁️ View", key=f"view_{job['id']}", type="primary"):
                            ...
                            st.rerun()
                with b2:
                    if job['status'] == "Completed" and not (job.get('calc_volume') and job.get('calc_sentiment') and job.get('calc_psych')):
...
if st.session_state.view_job_id is not None:
    ...
    if os.path.exists(result_file):
        ...
        with tabs[0]:
            for seg in segments:
                ...
                if "dbfs" in seg: 
                    ...
                    m_parts.append(f"📈 DR: {seg.get('dynamic_range',0)}dB")
                    m_parts.append(f"🏔️ Crest: {seg.get('crest_factor',0)}dB")
                    m_parts.append(f"🔇 Sil: {int(seg.get('silence_density',0)*100)}%")
                    m_parts.append(f"⏱️ {seg.get('wpm', 0)} wpm")
                if "sentiment" in seg: m_parts.append(f"🎭 {seg['sentiment']}")
                ...
                for f in seg.get("psych_flags", []):
                    flags_html += f"<span class='psych-flag'>{f}</span>"
                
                card_html = f"""<div class="segment-card" style="border-left-color: {color};">
        ...
        with tabs[1]:
            if not psych_stats:
                st.info("No psychological stats available. Click '✨ Add AI' to calculate them.")
            else:
                st.subheader("Gottman Ratios & Keyword Frequencies")
                st.markdown("Ratio of Positive statements to Negative statements (Criticism, Contempt, Defensiveness, Stonewalling, Dark Triad). Stable relationships target a 5:1 ratio.")
                
                for spk, stats in psych_stats.items():
                    spk_name = st.session_state.speaker_map.get(spk, spk)
                    color = get_speaker_color(spk)
                    
                    with st.expander(f"📊 {spk_name} (Gottman Ratio: {stats.get('gottman_ratio_desc', '0:0')} = {stats.get('gottman_ratio_val', 0)})", expanded=True):
                        ...
                        with col_b:
                            ...
                            st.write(f"❌ Criticism: {stats.get('four_horsemen_criticism', 0)}")
                            st.write(f"❌ Contempt: {stats.get('four_horsemen_contempt', 0)}")
                            st.write(f"❌ Defensiveness: {stats.get('four_horsemen_defensiveness', 0)}")
                            st.write(f"❌ Stonewalling: {stats.get('four_horsemen_stonewalling', 0)}")
                        with col_c:
                            st.write("**Psychological Markers**")
                            st.write(f"🎭 Dark Triad: {stats.get('dark_triad', 0)}")
                            st.write(f"🔗 Anxious Attachment: {stats.get('attach_anxious', 0)}")
                            st.write(f"🔗 Avoidant Attachment: {stats.get('attach_avoidant', 0)}")
        ...
        with tabs[3]:
            ...
            if psych_stats:
                ...
                for spk, stats in psych_stats.items():
                    spk_name = st.session_state.speaker_map.get(spk, spk)
                    txt_out.append(f"SPEAKER: {spk_name}")
                    ...
                    txt_out.append(f"Horsemen: Criticism ({stats.get('four_horsemen_criticism',0)}), Contempt ({stats.get('four_horsemen_contempt',0)}), Defensiveness ({stats.get('four_horsemen_defensiveness',0)}), Stonewalling ({stats.get('four_horsemen_stonewalling',0)})")
                    txt_out.append(f"Other: Dark Triad ({stats.get('dark_triad',0)}), Anxious ({stats.get('attach_anxious',0)}), Avoidant ({stats.get('attach_avoidant',0)})")
                    txt_out.append("-" * 40)
                txt_out.append("\n\n")
