...
<style>
    ...
    .timestamp { font-size: 0.8em; color: #888; font-family: monospace; vertical-align: middle; }
    .metrics-badge { float: right; font-size: 0.8em; color: #666; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px; margin-top: -5px; }
    ...
    [data-testid="stAudio"] {
        ...
        background-color: #1e1e1e; padding: 10px 20px; box-shadow: 0px -5px 15px rgba(0,0,0,0.5);
    }
    .main .block-container { padding-bottom: 120px !important; }
    
    /* Better button alignment */
    @media (max-width: 768px) {
        .action-row { display: flex; gap: 5px; flex-wrap: nowrap !important; overflow-x: auto; }
    }
</style>
""", unsafe_allow_html=True)

# Inject Global JS Event Listener to bypass Streamlit's onclick stripping
components.html("""
<script>
    ...
    for job in jobs:
        with st.container():
            ...
            with col_info:
                ...
                elif job['status'] == "Completed": pass # Hide 100% bar to save space
...
if st.session_state.view_job_id is not None:
    ...
    if os.path.exists(result_file):
        ...
        # Pre-calculate dynamic UI stats
            ...
            for i, seg in enumerate(segments):
                ...
                card_html = f"""<div class="segment-card" style="border-left-color: {color};">
                    <div style="margin-bottom: 8px; display: flex; align-items: center;">
                        <a class='inline-play' data-time='{start_sec}'>▶</a>
                    ...
                    <div style='margin-top:0.5rem;'>{flags_html}</div>
                </div>"""
